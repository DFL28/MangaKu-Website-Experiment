// Latest Page Script with Progressive Lazy Loading
...
// Load latest comics
async function loadLatestComics(page = 1) {
    ...
    grid.innerHTML = `
        <div class="updates-loading" style="grid-column: 1/-1;">
            ...
            <span>Memuat komik terbaru...</span>
        </div>
    `;
    ...
    try {
        ...
        const fetchPromises = [
            ...
            fetch(`${API_BASE}/api/updates?source=all&limit=2000&lang=id&sort=latest`)
                .then(res => res.json())
        ...
        console.log(`[Latest] Initial load: ${allData.length} manga`);

        if (allData.length === 0) {
            grid.innerHTML = `
                <div class="updates-empty" style="grid-column: 1/-1;">
                    Tidak ada komik terbaru saat ini
                </div>
            `;
        ...
        console.error('Error loading latest comics:', error);
        grid.innerHTML = `
            <div class="updates-empty" style="grid-column: 1/-1; color: var(--danger-color);">
                <i class="fas fa-exclamation-triangle"></i>
                Gagal memuat data. <button onclick="loadLatestComics(${page})" style="margin-left: 1rem; padding: 0.5rem 1rem; background: var(--primary-color); color: white; border: none; border-radius: 5px; cursor: pointer;">Coba Lagi</button>
            </div>
        `;
        pagination.style.display = 'none';
    }
...
async function loadMoreLatestData() {
    if (isLoadingMore) return;

    isLoadingMore = true;
    console.log(`[Latest] Loading more data from page ${nextScrapePage}...`);
    ...
    try {
        ...
        console.log(`[Latest] Added ${newData.length} new manga. Total: ${allData.length}`);
        ...
        console.error('[Latest] Error loading more data:', error);
    } finally {
        isLoadingMore = false;
    }
}
